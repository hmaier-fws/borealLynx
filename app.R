...
ui <-   navbarPage(title = "Boreal Lynx Project Collar Viewer",
                                                  ...
                                                  column(width=3,
                                                         ...
                                                         selectInput(inputId = "si.mgmtarea",
                                                                     ...
                                                                     selected = c("Tetlin Refuge",
                                                                                  ...
                                                                                  "Kanuti Refuge"),
                                                                     multiple = TRUE),
                                                         selectInput(inputId = "si.collar",
                                                                     label = "Collar Brand:",
                                                                     choices = c("Telonics" = "Telonics",
                                                                                 "ATS" = "ATS"),
                                                                     selected = c("ATS"),
                                                                     multiple = TRUE),
                   ...
                   server <- function(input, output) {
                     ...
                     dat <- reactive({                             # import the data (.csv)
                       ...
                       x$timestamp <- as.POSIXct(strptime(x$timestamp, format = "%m/%d/%Y %H:%M"))
                     ...
                     dat.sub.tbl <- reactive({
                       dat.sub() %>% group_by("CollarID" = ndowid) %>%  # Creates a summary table of the data
                         summarise(
                     ...
                     output$dt.animals <- DT::renderDataTable({        # Displays a summary table of the data
                       ...
                       DT::datatable(dat.sub.tbl(), options = list(pageLength = 5))
                     })
